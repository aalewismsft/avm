...
It supports both local README files and fetching from the Terraform Registry.

The script is specifically designed to work with Azure Verified Modules (AVM)
but should work with most Terraform modules that follow standard documentation formats.
...
and generate a JSON file with the extracted information in the 'output' directory.
...
def check_dependencies():
    """
    Check that all required third-party libraries are installed.
    
    This function verifies that the 'requests' library is available,
    which is needed to fetch data from the Terraform Registry.
    If missing, it prints installation instructions and exits.
    
    Returns:
        None
    
    Exits:
        With code 1 if dependencies are missing
    """
    missing_deps = []
    try:
        import requests
...
def ensure_folders_exist():
    """
    Create the working, output, and logs directories if they don't exist.
    
    This function creates the necessary directory structure for the script to operate:
    - working: for storing downloaded README.md files
    - output: for storing the generated JSON files
...
def setup_logging():
    """
    Configure the logging system for the script.
    
    Sets up logging to both:
    - A log file in the 'logs' directory
    - Standard output (console)
    
    The log level is set to DEBUG to capture all relevant information.
...
class ReadmeParser:
    ...
    1. Read a README.md file from a Terraform module
    2. Extract sections for required inputs, optional inputs, and outputs
    3. Parse the extracted sections into structured data
    4. Handle submodules if present in the module
    5. Convert the parsed data to JSON format
    
    The parser supports the standard README format used by Terraform modules
    on the Terraform Registry and in Azure Verified Modules (AVM).
    """
    ...
    def read_file(self) -> str:
        ...
        stores its content in the object, and performs a basic validation to
        check if it appears to be a Terraform module README.
        
        Returns:
            str: The content of the README.md file, or an empty string if file not found
            
        Logs:
            - INFO: When file is successfully read
            - WARNING: When file doesn't appear to be a Terraform module README
            - ERROR: When file is not found or other errors occur
        """
        try:
    ...
    def is_terraform_module_readme(self) -> bool:
        ...
        This method looks for common section headers and code block formats
        that typically appear in Terraform module documentation.
        
        Returns:
            bool: True if it appears to be a Terraform module README, False otherwise
            
        Algorithm:
            1. Checks for common section headers like "Required Inputs", "Outputs", etc.
            2. Checks for Terraform/HCL code blocks
            3. Returns True if at least 2 indicators are found
        """
    ...
    def extract_section(self, section_name: str) -> str:
        """
        Extract a section from the README.md file based on its heading.
        
        This method uses regex to find a section that starts with a given heading
        and ends at the next heading or end of file.
        
        Args:
            section_name (str): The name of the section to extract, e.g., "Required Inputs"
            
        Returns:
            str: The content of the section as a string, not including the heading.
                 Returns empty string if section not found.
                 
        Note:
            The regex pattern looks for a level-2 heading (##) with the exact section name,
            and captures all content until the next level-2 heading or end of file.
        """
    ...
    def extract_hcl_block(self, text: str) -> str:
        """
        Extract HCL (HashiCorp Configuration Language) code block from markdown text.
        
        Args:
            text (str): The text containing HCL code blocks, typically enclosed in
                        triple backticks with 'hcl' language specifier
            
        Returns:
            str: The HCL code block content without the surrounding backticks.
                 Returns empty string if no HCL block is found.
        """
        pattern = r"```hcl\n(.*?)```"
    ...
    def parse_input_entry(self, entry: str) -> Dict[str, Any]:
        ...
        This method extracts information about a single input variable, including:
        - Name
        - Description
        - Type
        ...
        Args:
            entry (str): The markdown text of a single input entry
            
        Returns:
            Dict[str, Any]: A dictionary with the parsed input information:
                {
                    "name": "input_name",
        ...
        Note:
            The parser expects input entries in the standard Terraform documentation format,
            with the input name in an HTML anchor tag followed by description, type, and
...
class TerraformRegistryFetcher:
    """
    Class to fetch module information from the Terraform Registry.
    
    This class provides functionality to:
    1. Connect to the Terraform Registry API
    2. Fetch module source repository information
    3. Download README.md files from GitHub repositories
    4. Handle submodule README files
    
    It acts as a bridge between the Terraform Registry and the local parser,
    allowing modules to be fetched directly by their registry path.
    """
    ...
    def __init__(self, module_name: str):
        ...
        Args:
            module_name (str): The module name in the format "namespace/name/provider"
                               For example: "Azure/compute/azurerm" or 
    ...
    def _parse_module_name(self, module_name: str) -> Tuple[str, str, str]:
        """
        Parse the module name into namespace, name, and provider components.
        
        Args:
            module_name (str): The module name in the format "namespace/name/provider"
            
        Returns:
            Tuple[str, str, str]: A tuple of (namespace, name, provider)
            
        Raises:
            ValueError: If the module name doesn't have exactly three components
    ...
    def _get_registry_module_url(self) -> str:
        """
        Get the URL for the module in the Terraform Registry web interface.
        
        This URL points to the human-readable registry page for the module.
        
        Returns:
            str: The full URL to the module's page in the Terraform Registry
    ...
    def _get_registry_api_url(self) -> str:
        ...
        This URL is used to fetch module metadata from the Registry API.
        
        Returns:
            str: The full URL to the module's API endpoint
            
        Example:
            "https://registry.terraform.io/v1/modules/Azure/avm-res-keyvault-vault/azurerm"
        """
    ...
    def fetch_and_save_readme(self) -> Optional[str]:
        ...
        try:
            ...
            script_dir = os.path.dirname(os.path.abspath(__file__))
            readme_filename = os.path.join(script_dir, "working", f"{module_filename}_README.md")
    ...
    def fetch_submodule_readme(self, submodule_path: str, submodule_name: str) -> Optional[str]:
        ...
        try:
            ...
            script_dir = os.path.dirname(os.path.abspath(__file__))
            readme_filename = os.path.join(script_dir, "working", f"{module_base}_{submodule_safe_name}_README.md")
...
def parse_readme_directly(readme_path):
    ...
    making it more robust for submodule parsing. It uses the ReadmeParser 
    internally but only for specific operations.
    
    Args:
        readme_path (str): Path to the README.md file
        
    Returns:
        Dict: A dictionary with parsed inputs and outputs:
            {
                "inputs": {
                    "input_name": {
                        "name": "input_name",
    ...
    Note:
        This function is primarily used for processing submodule READMEs
        which might have slight format differences from the main module README.
    """
    try:
...
def main():
    ...
    This function handles the overall workflow of the script:
    1. Sets up the necessary directory structure
    2. Determines if a module name was provided or if local README should be used
    3. For registry modules, fetches the README from GitHub
    4. Parses the README content to extract inputs, outputs, and submodules
    5. Generates and validates the JSON output
    6. Saves the result to the output directory
    
    The function doesn't take any arguments (uses sys.argv directly) and doesn't
    return any values (outputs to files and logs).
    
    Flow:
    - If module name is provided: fetch from Terraform Registry
    - If no module name: use local README.md
    - Parse the README for inputs, outputs, and submodules
    - Process any submodules if present
    - Generate JSON and save to output directory
    """
    try:
        ...
        if "_README.md" in readme_path:
            ...
            output_path = os.path.join(script_dir, "output", f"{module_name}.json")
        else:
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] in ['-h', '--help', 'help']:
        ...
        print("\nOutput:")
        print("  JSON file will be created in the '.github/output' folder")
        print("  README files will be downloaded to the '.github/working' folder")
        print("  Logs will be stored in the '.github/logs' folder")
        sys.exit(0)
